...
int KeySchedule (word *);
int SubByte (word);
void ByteSub_ShiftRow (word *, int);
void InversShiftRow_ByteSub (word *, int);
int MixColumn_AddRoundKey (word *, int, int);
int AddRoundKey_InversMixColumn (word *, int, int);
int AddRoundKey (word *, int);

void encrypt (word statemt[32], word key[32])
{
  int main_result;
  int round_val;
  int nb;
...
/* RotByte */
      if ((j % nk) == 0)
	{
	  temp[0] = SubByte (worda[1][j - 1]) ^ Rcon0[(j / nk) - 1];
	  temp[1] = SubByte (worda[2][j - 1]);
	  temp[2] = SubByte (worda[3][j - 1]);
	  temp[3] = SubByte (worda[0][j - 1]);
	}
      if ((j % nk) != 0)
	{
	  temp[0] = worda[0][j - 1];
	...
	for (i = 0; i < 4; ++i)
      ...
      for (i = 0; i < 4; ++i)
	worda[i][j] = worda[i][j - nk] ^ temp[i];
    }
  return 0;
}

/* ********* ByteSub & ShiftRow ********* */
void ByteSub_ShiftRow (word statemt[32], int nb)
{
  int temp;
...
void InversShiftRow_ByteSub (word statemt[32], int nb)
{
  int temp;
...
/* ******** MixColumn ********** */
int MixColumn_AddRoundKey (word statemt[32], int nb, int n)
{
  int ret[8 * 4], j;
  register int x;
  ...
  for (j = 0; j < nb; ++j)
	...
	ret[j * 4] ^= x;
      ret[j * 4] ^=
	statemt[2 + j * 4] ^ statemt[3 + j * 4] ^ worda[0][j + nb * n];
	...
	ret[1 + j * 4] ^= x;
      ret[1 + j * 4] ^=
	statemt[3 + j * 4] ^ statemt[j * 4] ^ worda[1][j + nb * n];
	...
	ret[2 + j * 4] ^= x;
      ret[2 + j * 4] ^=
	statemt[j * 4] ^ statemt[1 + j * 4] ^ worda[2][j + nb * n];
	...
	ret[3 + j * 4] ^= x;
      ret[3 + j * 4] ^=
	statemt[1 + j * 4] ^ statemt[2 + j * 4] ^ worda[3][j + nb * n];
    }
  for (j = 0; j < nb; ++j)
    {
      statemt[j * 4] = ret[j * 4];
...
/* ******** InversMixColumn ********** */
int AddRoundKey_InversMixColumn (word statemt[32], int nb, int n)
{
  int ret[8 * 4], i, j;
  register int x;
  ...
  for (j = 0; j < nb; ++j)
    {
      statemt[j * 4] ^= worda[0][j + nb * n];
      statemt[1 + j * 4] ^= worda[1][j + nb * n];
      statemt[2 + j * 4] ^= worda[2][j + nb * n];
      statemt[3 + j * 4] ^= worda[3][j + nb * n];
    }
  for (j = 0; j < nb; ++j)
    for (i = 0; i < 4; ++i)
      {
...
/* ******** AddRoundKey ********** */
int AddRoundKey (word statemt[32], int n)
{
  int j, nb;
